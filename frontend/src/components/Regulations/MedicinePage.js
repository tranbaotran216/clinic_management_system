        ...
        <div style={{ padding: 24 }}>
            ...
            <Modal title={editingMedicine ? 'Sửa thông tin thuốc' : 'Thêm thuốc mới'} open={isModalVisible} onOk={handleOk} onCancel={() => setIsModalVisible(false)} destroyOnClose>
                <Form form={form} layout="vertical" style={{ marginTop: 24 }} onFinish={handleOk}>
                    ...
                    <Form.Item name="cach_dung_mac_dinh_id" label="Cách dùng" rules={[{ required: true, message: 'Vui lòng chọn cách dùng!' }]}><Select placeholder="Chọn cách dùng" allowClear>{usages.map(u => <Option key={u.id} value={u.id}>{u.ten_cach_dung}</Option>)}</Select></Form.Item>
                    <Row gutter={16}>
                        <Col span={12}><Form.Item name="don_vi_tinh_id" label="Đơn vị tính (*)" rules={[{ required: true, message: 'Vui lòng chọn đơn vị tính!' }]}><Select placeholder="Chọn đơn vị tính">{units.map(u => <Option key={u.id} value={u.id}>{u.ten_don_vi_tinh}</Option>)}</Select></Form.Item></Col>
